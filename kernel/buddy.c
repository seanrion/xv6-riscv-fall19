...
int bit2_isset(char *array, int index) {
  index >>= 1;
  char b = array[index/8];
  char m = (1 << (index % 8));
  return (b & m) == m;
...
void bit2_set(char *array, int index) {
  index >>= 1;
  char b = array[index/8];
  char m = (1 << (index % 8));
  array[index/8] = (b ^ m);
...
bd_malloc(uint64 nbytes)
{
  ...
  bit2_set(bd_sizes[k].alloc, blk_index(k, p));
  for(; k > fk; k--) {
    ...
    bit2_set(bd_sizes[k-1].alloc, blk_index(k-1, p));
...
bd_free(void *p) {
  ...
  for (k = size(p); k < MAXSIZE; k++) {
    ...
    bit2_set(bd_sizes[k].alloc, bi);
    //if (bit_isset(bd_sizes[k].alloc, buddy)) {  // is buddy allocated?
    if (bit2_isset(bd_sizes[k].alloc, buddy)) { 
...
#define addr_in_range(addr,left,right,size) ((addr>=left)&&((addr+size)<right))
...
bd_initfree_pair(int k, int bi,void *left,void *right) {
  ...
  if(bit2_isset(bd_sizes[k].alloc,bi)){
